...
// Minimal local shape — matches MediaPipe's landmark point structure
...
const detectForVideo = vi.fn().mockReturnValue({ landmarks: [] as HandLandmark[][] });
const close = vi.fn();
...
vi.mock('@mediapipe/tasks-vision', () => ({
  ...
  HandLandmarker: {
    createFromOptions: (...args: unknown[]): ReturnType<typeof createFromOptions> =>
...
describe('useHandTracking', () => {
  it('falls back to the CPU delegate when the GPU delegate fails to create', async () => {
    ...
    expect(firstCallOptions.baseOptions.delegate).toBe('GPU');
    expect(secondCallOptions.baseOptions.delegate).toBe('CPU');
  });
